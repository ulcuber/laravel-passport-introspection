...
use crate::{
    app::AppState,
    database::AccessTokenRepository,
    jwt::validate_jwt,
};

use crate::responses::{unauthorized_response, server_error_response};
...
pub async fn handle(
    ...
    headers.insert(
        "X-Gateway-Secret",
        HeaderValue::from_str(&state.config.gateway_secret).unwrap_or_else(|_| HeaderValue::from_static(""))
    );
