...
use laravel_passport_introspection::config::Config;
...
async fn main() -> anyhow::Result<()> {
    ...
    if !latencies.is_empty() {
        ...
        println!(
            "Latency p50: {:.2}ms",
            latencies[latencies.len() / 2] * 1000.0
        );
        println!(
            "Latency p90: {:.2}ms",
            latencies[latencies.len() * 9 / 10] * 1000.0
        );
        println!(
            "Latency p99: {:.2}ms",
            latencies[latencies.len() * 99 / 100] * 1000.0
        );
    }
